...
public class StandardizerPlugin : IPlugin
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var cacheDataArray = await ParseInputCacheDataAsync(inputData, context);
    ...
    /// </summary>
    private async Task<JsonElement[]> ParseInputCacheDataAsync(string inputData, HierarchicalLoggingContext context)
    {
        _logger.LogDebugWithHierarchy(context, "Parsing input compressed file cache data array from FileReader");
        ...
        try
        {
            ...
            _logger.LogInformationWithHierarchy(context, "Successfully parsed compressed file cache data array with {ItemCount} items", jsonArray.Length);
            return await Task.FromResult(jsonArray);
        }
        catch (JsonException ex)
        {
            _logger.LogErrorWithHierarchy(context, ex, "Failed to parse input cache data as JSON array");
