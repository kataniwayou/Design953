...
public class AudioConverterPlugin : IPlugin
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var cacheDataArray = await ParseInputCacheDataAsync(inputData, pluginContext);
    ...
    private async Task<JsonElement[]> ParseInputCacheDataAsync(string inputData, HierarchicalLoggingContext context)
    {
        try
        {
            ...
            _logger.LogInformationWithHierarchy(context, "Successfully parsed {ItemCount} compressed file cache data items", jsonArray.Length);
            return await Task.FromResult(jsonArray);
        }
        catch (Exception ex)
        {
            _logger.LogErrorWithHierarchy(context, ex, "Failed to parse input cache data");
            throw;
        }
    }
